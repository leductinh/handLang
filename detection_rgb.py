...
model = load_model('models/mymodel_rgb.h5')
...
while camera.isOpened():
    ...
    if isBgCaptured == 1:
        ...
        cv2.imshow('original1', cv2.resize(blur, dsize=None, fx=0.5, fy=0.5))  # anh goc

        ret, thresh = cv2.threshold(blur, threshold, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

        cv2.imshow('thresh', cv2.resize(thresh, dsize=None, fx=0.5, fy=0.5))  # anh da remove background
